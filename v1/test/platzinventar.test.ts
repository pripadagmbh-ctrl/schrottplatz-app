...
import { ABLADE_SPUR_X, KIPP_SPUR_X } from "../src/delivery/routes";
...
describe("Der Müllcontainer ist ein Container, kein Bauwerk", () => {
  ...
  it("sein Startplatz ist frei gewählt, aber begründet", () => {
     ...
     * drei Dinge können: im Schwenkband liegen (sonst kann der Spieler am
     * ersten Tag nichts hineinwerfen), die Kipperspur frei lassen und der
     * Buntmetall-Mulde nicht im Weg stehen.
     */
    ...
    const kipperFlanke = KIPP_SPUR_X - 1.55; // halbe Wagenbreite
    expect(
      kipperFlanke - ostkante,
      `nur ${(kipperFlanke - ostkante).toFixed(2)} m bis zur Kipperflanke (Schranke 1,40)`
    ).toBeGreaterThan(1.4);
    // Und weit weg von der Rückfahrspur zum Abladeplatz.
    expect(ABLADE_SPUR_X - ostkante).toBeGreaterThan(5.0);
  });
