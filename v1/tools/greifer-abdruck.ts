/**
 * Der ABDRUCK der Sichelkralle — eine Zahlenreihe, die den ganzen Greifer
 * beschreibt, wie er sich bewegt und was er fuer seinen Korb haelt.
 *
 * Wofuer: E-057 legt alles, was am Greifer haengt, hinter eine Schnittstelle.
 * Die haerteste Bedingung dabei ist, dass sich die Sichelkralle NICHT aendert.
 * „Sieht gleich aus" reicht dafuer nicht. Dieses Werkzeug nimmt den Abdruck
 * VOR dem Umbau (Stand aus der Versionsverwaltung) und danach; verglichen wird
 * Zahl fuer Zahl.
 *
 * Was im Abdruck steckt:
 *   - 300 Schritte am kopflos gebauten Bagger: Arm senken, zupacken, oeffnen
 *   - je Schritt: Pose der Spinne, Spreizung jeder Schale, Sensormitte
 *   - die Weltlage JEDES Netzes unter der Spinne (Modell)
 *   - die Weltlage JEDES Krallenkolliders (Physik)
 *   - ein Gitter aus 2.000 Punkten durch `isInsideGrapple` (Korb)
 *
 * Aufruf:  npx vite-node tools/greifer-abdruck.ts > abdruck.json
 *
...
async function main(): Promise<void> {
  ...
  /* Ein Brocken unter der Spinne, damit auch das Blockieren mitlaeuft. */
  ...
  /* Das Modell: jedes Netz unter der Spinne mit seiner Weltlage. */
  ...
  bagger.grappleGroup.traverse((n) => {
    ...
    netze.push(
      `${m.geometry.type}:${m.geometry.getAttribute("position").count}`
    );
    zahlen.push(v.x, v.y, v.z, q.x, q.y, q.z, q.w, s.x, s.y, s.z);
  });

  /* Die Physik: jeder Kollider am Greiferkoerper. */
  ...
  /* Der Korb: ein Gitter durch `isInsideGrapple`, bei drei Stellungen. */
  const korb: number[] = [];
    ...
    for (let iy = 0; iy <= 24; iy++) {
      for (let ir = 0; ir <= 12; ir++) {
        for (let ia = 0; ia < 4; ia++) {
          ...
          korb.push(
            bagger.isInsideGrapple(
              v.set(mitte.x + Math.sin(a) * r, y, mitte.z + Math.cos(a) * r)
            )
              ? 1
              : 0
          );
