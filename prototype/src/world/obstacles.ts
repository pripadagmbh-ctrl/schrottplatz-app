...
export const STATIC_OBSTACLES: Obstacle[] = [
  ...
  { x: 0, z: -HZ, hw: HX, hd: WALL_T / 2, label: "Südwand" },
  { x: -HX, z: 0, hw: WALL_T / 2, hd: HZ, label: "Westwand" },
  { x: HX, z: 0, hw: WALL_T / 2, hd: HZ, label: "Ostwand" },
  ...
  { x: 4.9, z: -4.2, hw: 2.3, hd: 1.6, label: "Box VA" },
  { x: 4.9, z: -0.3, hw: 2.3, hd: 1.6, label: "Box Alu" },
  { x: 4.9, z: 3.6, hw: 2.3, hd: 1.6, label: "Box Kupfer" },
  { x: 4.9, z: 7.5, hw: 2.3, hd: 1.6, label: "Box Kabel" },

  // --- Schere und Presse, südlich hinter dem Bagger ---
  { x: 0, z: -9.5, hw: 4.4, hd: 3.8, label: "Schere" },
];

/** Liegt (x,z) in einem festen Bauwerk? `pad` erweitert es um einen Sicherheitsrand. */
export function hitsObstacle(x: number, z: number, pad = 0): Obstacle | null {
  for (const o of STATIC_OBSTACLES) {
    if (Math.abs(x - o.x) < o.hw + pad && Math.abs(z - o.z) < o.hd + pad) return o;
  }
  return null;
}
